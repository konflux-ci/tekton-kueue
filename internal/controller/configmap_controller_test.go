...
var _ = Describe("ConfigMapReconciler", func() {
	...
	Describe("Reconcile", func() {
		It("should return success when ConfigMap is not found", func() {
			fakeClient := fake.NewClientBuilder().WithScheme(s).Build()
			reconciler = &ConfigMapReconciler{Client: fakeClient, Store: store}

			result, err := reconciler.Reconcile(testCtx, ctrl.Request{NamespacedName: nsName})
			Expect(err).NotTo(HaveOccurred())
			Expect(result).To(Equal(ctrl.Result{}))
		})

		It("should return error when Client.Get fails with a non-NotFound error", func() {
		...
		It("should return success when ConfigMap exists but config key is missing", func() {
			cm := &corev1.ConfigMap{
		...
		It("should requeue when config YAML is invalid", func() {
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      common.ConfigMapName,
					Namespace: "tekton-kueue",
		...
		It("should requeue when config validation fails (empty queue name)", func() {
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      common.ConfigMapName,
					Namespace: "tekton-kueue",
			...
			result, err := reconciler.Reconcile(testCtx, ctrl.Request{NamespacedName: nsName})
			Expect(err).To(MatchError("queue name is not set in the PipelineRunCustomDefaulter"))
			Expect(result.RequeueAfter).To(Equal(10 * time.Second))
		})

		It("should update the store successfully with valid config", func() {
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      common.ConfigMapName,
					Namespace: "tekton-kueue",
		...
		It("should update the store with CEL mutators when expressions are provided", func() {
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      common.ConfigMapName,
					Namespace: "tekton-kueue",
